...
class GaussianMixture():
    ...
    def create_graph(self,points,log_assignements,t):
        ...
        for i in range(self.n_components):
            ...
            x_points[i] = np.asarray([points[j] for j in range(n_points) if (np.argmax(log_assignements[j])==i)])     
            y_points[i] = np.asarray([points[j] for j in range(n_points) if (np.argmax(log_assignements[j])==i)])
            ...
            for i in range(self.n_components):
                ...
                if self.log_prior_prob[i] > -3:
                    ...
                    if self.covariance_type=="full":
                        ...
                        covariance = self.cov[i]*np.eye(dim,dim)
                    
                    ell = utils.ellipses(covariance,self.means[i])
                    ell = utils.ellipses_multidimensional(covariance,self.means[i],d)
                    ax.add_artist(ell)
                      
            titre = directory + '/figure_' + str(t) + '_proj_' + str(d)
            plt.savefig(titre)
            plt.close("all")
...
#    points_test = points_data
    ...
    for i in range(10):
        ...
        GMM.create_graph(points_data,log_assignements_data,str(i) + "_MFCC")
    #        GMM.create_graph_log(str(i) + "_MFCC")
        print()